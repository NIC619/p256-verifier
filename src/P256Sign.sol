...
// This contract copy and modify P256Verifier contract. It is used to generate P256 signature. 
contract P256Sign {
    ...
    /**
     * @dev ECDSA sign for testing only.
     */
    function ecdsa_sign(
        bytes32 message_hash,
        uint256 k, // k is usually derived from the message hash according to RFC-6979 but for testing a random k will do
        uint256 privKey
    ) public view returns (uint256 r, uint256 s) {
        // Calculate the curve point k.G (abuse ecmulmul add with v=0)
        r = ecZZ_mulmuladd(0, 0, k, 0);
        r = addmod(0, r, n);
        s = mulmod(nModInv(k), addmod(uint256(message_hash), mulmod(r, privKey, n), n), n);
        require(r != 0 && s != 0, "Invalid signature");
